...
subject.LongitudinalData <- function(ld_df, id) {
  position <- which(ld_df[["id"]] == id)
  if (length(position) == 0)
    return(NULL)
  structure(list(id = id, data = ld_df[["data"]][[position]]), class = "Subject")
}
...
summary.Subject <- function(object) {
  output <- object[["data"]] %>% 
    ...
    spread(room, value) %>% 
    as.data.frame
  structure(list(id = object[["id"]], output = output), class = "Summary")
}
...
summary.Room <- function(rm_object) {
  output <- summary(rm_object[["data"]][["value"]])
  structure(list(id = rm_object[["id"]],
